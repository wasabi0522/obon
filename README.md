...
- **Process tree detection** — finds target panes even when the command has spawned subprocesses
...
|----------|----------|---------|-------------|
| `OBON_TARGET_CMD` | Yes | — | Command name to match (foreground process and process tree) |
| `OBON_MIN_PANE_WIDTH` | No | `80` | Minimum pane width (columns). Join warns if each pane would be narrower than this value |
