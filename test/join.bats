...
@test "join: fails when obon window already exists" {
  tmux -L "$OBON_TMUX_SOCKET" new-window -t "hs/org/repo" -n "obon"
  run "$OBON_BIN" join -y
  assert_failure
  assert_output --partial "obon window already exists"
}
...
@test "join --all: fails when obon session already exists" {
  tmux -L "$OBON_TMUX_SOCKET" new-session -d -s "obon" -n "obon"
  run "$OBON_BIN" join --all -y
  assert_failure
  assert_output --partial "obon session already exists"
}
